...
<script type="text/javascript" src="<?php echo get_stylesheet_directory_uri()."/js/moment-with-locales.min.js"; ?>"></script>
<script type="text/javascript" src="<?php echo get_stylesheet_directory_uri()."/js/ion.calendar.js"; ?>"></script>   
   <script>
	jQuery(function(){
		jQuery('.datepicker').attr("data-lang","en");
		jQuery('.datepicker').attr("data-years","1900-2040");
		jQuery('.datepicker').attr("data-format","DD-MM-YYYY");	
		setTimeout(explode,500);		
		function explode()
		{  		
			jQuery(".datepicker").each(function()
			{
				jQuery(".datepicker").ionDatePicker();
			});				
		}
	});
	jQuery(document).ready(function(){
    	jQuery('#datepicker').keypress(function (event) {
            return false;
        });
    });
</script>
<style>
.page-id-224 .ic__datepicker {left:40% !important;}
.page-id-224 .ic__days .ic__day:hover {background: #d35746 none repeat scroll 0 0 !important;color: #fff;}
</style>
<?php } ?>
<style>
.page-id-176 .ctm_accordion .accordion .question::after{width:0px;}
</style>
<?php if(get_the_ID() == "221"){ 
 } ?>
<script>		 
jQuery(document).ready(function(){
	jQuery(".ctm_img").click(function(){
		var url = "http://fuelyourlife.com.au/referral-form/";
		window.location = url;
	});
});
</script>
