 ...
 */
class CF7GSC_Logger_File extends CF7GSC_Logger_Abstract
{
  /**
  ...
  public function __construct(CF7GSC_Client $client)
  {
    ...
    $file = $client->getClassConfig('CF7GSC_Logger_File', 'file');
    ...
    $mode = $client->getClassConfig('CF7GSC_Logger_File', 'mode');
    if (!$mode) {
      $this->mode = $mode;
    }

    $this->lock = (bool) $client->getClassConfig('CF7GSC_Logger_File', 'lock');
    $this->file = $file;
  }
  ...
  protected function write($message)
  {
    if (is_string($this->file)) {
      ...
      throw new CF7GSC_Logger_Exception('File pointer is no longer available');
    }
