 ...
 */
abstract class CF7GSC_Logger_Abstract
{
  /**
   * Default log format
   */
  ...
  /**
   * @param CF7GSC_Client $client  The current Google client
   */
  public function __construct(CF7GSC_Client $client)
  {
    $this->setLevel(
        $client->getClassConfig('CF7GSC_Logger_Abstract', 'level')
    );

    $format = $client->getClassConfig('CF7GSC_Logger_Abstract', 'log_format');
    $this->logFormat = $format ? $format : self::DEFAULT_LOG_FORMAT;

    $format = $client->getClassConfig('CF7GSC_Logger_Abstract', 'date_format');
    $this->dateFormat = $format ? $format : self::DEFAULT_DATE_FORMAT;

    $this->allowNewLines = (bool) $client->getClassConfig(
        'CF7GSC_Logger_Abstract',
        'allow_newlines'
    );
  }
   ...
   * @throws CF7GSC_Logger_Exception If $level is invalid
   */
  protected function normalizeLevel($level)
  {
    ...
    throw new CF7GSC_Logger_Exception(
